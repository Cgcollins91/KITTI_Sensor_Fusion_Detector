...
def plot_img_and_lidar(uv, Z_cam, img, labels, bound_mask, Z_mask, filter=True, pred_boxes=None):
    """
    ...
    if pred_boxes is not None:
        for bbox in pred_boxes:
            ...
            bbox[3]-bbox[1],    # Height
            linewidth=2,        # Bounding box line width
            edgecolor='r',      # Bounding Box Line Color
...
model_path  = working_folder + '/model.pth'
model       = initiate_eval_model(model_path)

# %% Plot Detecter Model on top of Kitti labels

conf_thresh = 0.5
pred_box, pred_scores = predict_image_car_bbox(model, img, conf_thresh=conf_thresh)
plot_img_and_lidar(uv, Z_cam, img, labels, bound_mask, Z_mask, filter=True, pred_boxes=pred_box)
